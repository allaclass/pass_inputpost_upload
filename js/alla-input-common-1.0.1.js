// 제    목 : alla-input-common-1.0.1.js
// 작 성 자 : 김현수
// 작 성 일 : 2022.01.09
// 
// 업데이트 : alla-input-common-3.0.1.js copy
// 업데이트 : 파일명 변경 (alla-input-common-1.0.1.js)
...
var txt_object_etc = document.getElementById('txt_object_etc');
var txt_office = document.getElementById('txt_office');
var txt_year = document.getElementById('txt_year');
var txt_no = document.getElementById('txt_no');
...
function info_input(){
    ...
    var infoObjectEtc = infoData.indexOf('[전체과목]');
    var infoOffice = infoData.indexOf('[시행기관]');
    var infoYear = infoData.indexOf('[연도]');
    var infoNo = infoData.indexOf('[회차]');
    var infoDate = infoData.indexOf('[일시]');
    var infoSNum = infoData.indexOf('[시작번호]');
    var infoENum = infoData.indexOf('[종료번호]');
    var infoThumb = infoData.indexOf('[썸네일]');
    var infoAnswer = infoData.indexOf('[정답]');
    var infoLength = infoData.length;

    var infoLicenseData = (infoData.substring(infoLicense+7, infoObject)).replace(/(^\s*)|(\s*$)/gi, "");
    var infoObjectData = (infoData.substring(infoObject+7, infoObjectEtc)).replace(/(^\s*)|(\s*$)/gi, "");
    var infoObjectEtcData = (infoData.substring(infoObjectEtc+7, infoOffice)).replace(/(^\s*)|(\s*$)/gi, "");
    var infoOfficeData = (infoData.substring(infoOffice+7, infoYear)).replace(/(^\s*)|(\s*$)/gi, "");
    var infoYearData = (infoData.substring(infoYear+5, infoNo)).replace(/(^\s*)|(\s*$)/gi, "");
    var infoNoData = (infoData.substring(infoNo+5, infoDate)).replace(/(^\s*)|(\s*$)/gi, "");
    var infoDateData = (infoData.substring(infoDate+5, infoSNum)).replace(/(^\s*)|(\s*$)/gi, "");
    var infoSNumData = (infoData.substring(infoSNum+7, infoENum)).replace(/(^\s*)|(\s*$)/gi, "");
    var infoENumData = (infoData.substring(infoENum+7, infoThumb)).replace(/(^\s*)|(\s*$)/gi, "");
    var infoThumbData = (infoData.substring(infoThumb+6, infoAnswer)).replace(/(^\s*)|(\s*$)/gi, "");
    var infoAnswerData = (infoData.substring(infoAnswer+5, infoLength)).replace(/(^\s*)|(\s*$)/gi, "");

    txt_license.value = infoLicenseData;
    txt_object.value = infoObjectData;
    txt_object_etc.value = infoObjectEtcData;
    txt_office.value = infoOfficeData;
    txt_year.value = infoYearData;
    txt_no.value = infoNoData;
...
// auto post_title
function post_title(){
    txt_title.value = '20'+txt_year.value+'년 '+txt_no.value+'회 '+txt_license.value+' ('+txt_object.value+') 필기 / 올에이패스 모의고사';
}
...
function html_shortcut(){
    ...
    src_shortcut += '\t\t</div>\n';
    src_shortcut += '\t\t<div>\n';
    src_shortcut += '\t\t\t'+txt_license.value+' 1과목 '+txt_object_etc1.value+'<br>\n';
    src_shortcut += '\t\t\t'+txt_license.value+' 2과목 '+txt_object_etc2.value+'<br>\n';
    src_shortcut += '\t\t\t'+txt_license.value+' 3과목 '+txt_object_etc3.value+'<br>\n';
    src_shortcut += '\t\t\t'+txt_license.value+' 4과목 '+txt_object_etc4.value+'<br>\n';
    src_shortcut += '\t\t\t'+txt_license.value+' 5과목 '+txt_object_etc5.value+'<br>\n';
    src_shortcut += '\t\t</div>\n';
    src_shortcut += '\t</a>\n';
    src_shortcut += '</div>\n';
    src_shortcut += '<!-- // 타년도 문제 바로가기 영역 -->\n';
